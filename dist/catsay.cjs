...
function numberArrayMax(values) {
    return values.reduce((max, value) => (max > value ? max : value), 0);
}
function computeBreakPoint(text, maxWidth = 40) {
...
function border(textList, breakPoint, padding = 0) {
    ...
    textList.forEach((line) => {
        result.push(`│${generateChar(' ', padding)}${line}${generateChar(' ', breakPoint - stringWidth(line))}${generateChar(' ', padding)}│`);
    });
...
function bubble(text, { padding = 2, maxWidth, W, type = 'say', boxStyle, B } = {}) {
...
function normalizeInput(input, fallbackOptions) {
